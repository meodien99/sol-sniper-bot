...
This Tool is designed to automate the buying and selling of tokens on the Solana network.
Basic logics here is listen new pool created in Raydium AMM and if that token matches the filter sets to execute trades based on the predefined parameters and strategies set by the user.
